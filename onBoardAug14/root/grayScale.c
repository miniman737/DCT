...
/* ----------------------------------------------------------------
 * Pure-C Loeffler, for the firmware-vs-software comparison
 *
 * Structurally identical to dct_2d_loeffler() in your source: same
 * stages, same coefficients, same Q10 rounding, same deferred gain
 * shift. The ONLY difference is butterfly_c() below, which performs
 * the 3-multiply rotation in plain C instead of hand-scheduled ARM
 * assembly. Isolating that one function is the whole point -- any
 * timing gap between SW-LOEFFLER and FW-LOEFFLER is attributable to
 * the asm and nothing else.
 *
 * The constants and rotator_table come from your file, which is
 * included above in the same translation unit, so there is no risk
 * of the two versions drifting apart on coefficients.
 *
 * The two must produce BIT-IDENTICAL output. The report checks this
 * and flags any mismatch -- if they ever differ, the assembly is
 * computing something other than what it claims.
 * ---------------------------------------------------------------- */
                        ...
                        uint8_t rotator)
{
    ...
    /* Mirrors the asm exactly: the rounding constant is folded into
     * tmp before both adds, matching "add tmp, tmp, #512". */
...
" invisible in a reconstructed image.\n\n");
    ...
    /* ---- firmware vs software: the asm isolation ---- */
    fprintf(f,
"----------------------------------------------------------------\n"
" FIRMWARE VS SOFTWARE -- WHAT THE INLINE ASSEMBLY BUYS\n"
"----------------------------------------------------------------\n"
" SW-LOEFFLER and FW-LOEFFLER run the identical algorithm: same\n"
" stages, same Q10 coefficients, same rounding, same deferred gain\n"
" shift. They differ in exactly one function -- the 3-multiply\n"
" butterfly rotation -- which SW does in plain C and FW does in\n"
" hand-scheduled ARM assembly. Any gap between them is the assembly\n"
" and nothing else.\n"
"\n");

    if (im[1].valid && im[2].valid) {
        double sw = im[1].median_s, fw = im[2].median_s;
        double speedup = sw / fw;
        double delta   = (sw - fw) * 1e3;

        fprintf(f, " %-16s %12s %14s %14s\n",
                "", "MEDIAN (ms)", "PER BLOCK(us)", "PER BUTTERFLY(ns)");
        /* 6 butterflies per 1D transform, 16 transforms per block */
        long bf = (long)img->blocks * 16 * 6;
        fprintf(f, " %-16s %12.3f %14.3f %14.1f\n", "SW-LOEFFLER (C)",
                sw * 1e3, sw * 1e6 / img->blocks, sw * 1e9 / (double)bf);
        fprintf(f, " %-16s %12.3f %14.3f %14.1f\n", "FW-LOEFFLER (asm)",
                fw * 1e3, fw * 1e6 / img->blocks, fw * 1e9 / (double)bf);

        fprintf(f, "\n Butterfly rotations executed: %ld "
                   "(%d blocks x 16 transforms x 6)\n", bf, img->blocks);

        if (speedup > 1.02) {
            fprintf(f,
"\n RESULT: the assembly is %.2fx faster, saving %.3f ms per frame.\n"
"\n"
" The 3-multiply rotation has two independent multiplies (il*s1 and\n"
" iu*s2) that do not depend on each other. Scheduling them into\n"
" adjacent issue slots lets the second start before the first\n"
" retires, so the rotation is bounded by multiplier latency rather\n"
" than by instruction count. The C compiler may not find this if it\n"
" cannot prove the operands are independent, or if it spills the\n"
" coefficient struct to the stack instead of keeping it in\n"
" registers.\n", speedup, delta);
        } else if (speedup < 0.98) {
            fprintf(f,
"\n RESULT: the assembly is SLOWER (%.2fx, costing %.3f ms per frame).\n"
"\n"
" This is a real and reportable outcome, not a mistake. At -O2 the\n"
" compiler already schedules the two independent multiplies well,\n"
" and an asm block is an optimisation barrier: the compiler cannot\n"
" inline through it, cannot keep values in registers across it, and\n"
" must materialise the coefficient struct into the constrained\n"
" registers on every call. Hand assembly beats the compiler far less\n"
" often on a modern toolchain than the folklore suggests.\n"
"\n"
" Worth checking before writing this up: compare the generated code\n"
" for both with\n"
"     gcc -O2 -S -fverbose-asm dct_loeffler.c\n"
" and count instructions in the rotation.\n", speedup, -delta);
        } else {
            fprintf(f,
"\n RESULT: no significant difference (%.2fx). At -O2 the compiler is\n"
" already emitting essentially the same schedule you wrote by hand.\n"
" That is a legitimate finding -- it says the C was not leaving\n"
" performance on the table, and it is worth stating explicitly\n"
" rather than presenting the asm as an unquantified win.\n", speedup);
        }

        /* Bit-identity check -- the correctness half of the claim */
        diff_t d;
        int xp = compare_best(im[1].out, im[2].out, img->blocks, &d);
        fprintf(f, "\n Output equivalence: ");
        if (d.max_abs == 0 && !xp) {
            fprintf(f, "BIT-IDENTICAL across all %ld coefficients.\n"
                       " The assembly computes exactly what the C computes, so the\n"
                       " speed comparison above is measuring the same work.\n", ncoef);
        } else {
            fprintf(f,
"MISMATCH -- max difference %d%s.\n"
"\n"
" *** This matters more than the timing. *** The two should be bit-\n"
" identical: same algorithm, same constants, same rounding. A\n"
" difference means the assembly is not computing what the C\n"
" computes, so any speed advantage is measuring different work.\n"
" Check the rounding constant (the asm folds +512 into tmp before\n"
" both adds) and the shift (asr #10, arithmetic not logical).\n",
                    d.max_abs, xp ? " (and transposed)" : "");
        }
    } else {
        fprintf(f, " One of the two Loeffler variants did not run.\n");
    }
    fprintf(f, "\n");
...
"   SW-LOEFFLER   11 integer multiplies, 29 adds, plain C\n"
"   FW-LOEFFLER   same, with the rotation hand-scheduled in asm\n"
