...
// Implements 2-slot firmware butterfly rotation as inline assembly within the butterfly_fp function. Scheduled for ARM Cortex-A8. 
...
// Butterfly firmware routine implementation
// Input: upper = I', lower = I" - input values to be rotated
// Output: out_upper = O', out_lower = O" - rotated values
// 2-slot firmware butterfly rotation (inline assembly), using 3-multiply rotation with pre-calculated coefficients. Sceduled for ARM Cortex-8.
static void butterfly_fp(int16_t in_upper, int16_t in_lower, int16_t *out_upper, int16_t *out_lower, uint8_t rotator)
{
	int32_t tmp, t_upper, t_lower;
    ...
    int32_t cos_c = coef.cos_coef, s1 = coef.simplified_1, s2 = coef.simplified_2;      // Typecast to int32_t to prevent overflow during multiplication

    __asm__(
        "mul %[tu], %[il], %[s1]\n\t"		        // CC0: t_upper = in_lower * simplified_1
        "add %[tmp], %[iu], %[il]\n\t"		        // CC0: tmp = in_upper + in_lower

        "mul %[tl], %[iu], %[s2]\n\t"		        // CC1: t_lower = in_upper * simplified_2

        "mul %[tmp], %[tmp], %[cos]\n\t"	        // CC2: tmp = cos_coef * (in_upper + in_lower)

        "add %[tmp], %[tmp], #512\n\t"            // CC5: tmp = tmp + 512 (for rounding)

        "add %[tu], %[tu], %[tmp]\n\t"		        // CC6: t_upper = t_upper + tmp
        "add %[tl], %[tl], %[tmp]\n\t"		        // CC6: t_lower = t_lower + tmp

        "asr %[tu], %[tu], #10\n\t"	                // CC7: t_upper = t_upper >> dct_fp_precision
        "asr %[tl], %[tl], #10\n\t"	                // CC7: t_lower = t_lower >> dct_fp_precision


        : [tu]"=&r"(t_upper), [tl]"=&r"(t_lower), [tmp]"=&r"(tmp)
        : [iu]"r"(iu), [il]"r"(il), [cos]"r"(cos_c), [s1]"r"(s1), [s2]"r"(s2)
    );

    *out_upper = (int16_t) t_upper;
    *out_lower = (int16_t) t_lower;
}
